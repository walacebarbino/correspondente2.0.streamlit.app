...
from streamlit_gsheets import GSheetsConnection  # Importante!
...
if check_password():
    ...
    # --- 2. CONEXÃO REAL (USANDO O JSON DAS SECRETS) ---
    # Aqui é onde a mágica acontece. Ele usa o gspread por baixo dos panos.
    conn = st.connection("gsheets", type=GSheetsConnection)
    URL_PLANILHA = "https://docs.google.com/spreadsheets/d/1n6529TSBqYhwqAq-ZwVleV0b9q0p38PSPT4eU1z-uNc/edit"

    # Lendo os dados (ttl=0 força ler a nuvem toda vez)
    ...
    with tab_bi:
        ...
        if not df.empty:
            ...
            m2.metric("Volume Total", formatar_br(df.iloc[:, 4].sum())) # Coluna 4 é o Valor
            ...
            # Ajuste de nomes para o agrupamento funcionar
    ...
    with tab_carteira:
        ...
        with st.container(height=500): # ROLAGEM AMARELA (REGRA 1)
            for i, r in df.iterrows():
                c = st.columns([3, 2, 1, 0.5])
                c[0].write(f"**{r.iloc[1]}**") # Nome
                c[1].write(f"{r.iloc[7]}") # Status
                c[2].write(formatar_br(r.iloc[4])) # Valor
                
                # BOTÃO DE EXCLUIR REAL
