...
if check_password():
    ...
    # TRATAMENTO DE ERRO DE DATA (nan)
    if not df.empty:
        df['DATA_DT'] = pd.to_datetime(df.iloc[:, 0], dayfirst=True, errors='coerce')
        # Se a data for inválida, assume a data de hoje para não exibir 'nan'
    ...
    tab_bi, tab_carteira = st.tabs(["📊 Dashboard", "📋 Carteira"])

    if not df.empty:
        with tab_bi:
            # (Mantido o BI Duplo e Tabela Azul conforme Regra 1)
            ...
            g1, g2 = st.columns(2)
            with g1: st.plotly_chart(px.bar(df, x='MÊS_ANO', y=df.columns[4], color=df.columns[6], title="Volume por Enquadramento"), use_container_width=True)
            with g2: st.plotly_chart(px.bar(df, x='MÊS_ANO', y=df.columns[4], color=df.columns[7], title="Volume por Status"), use_container_width=True)
        ...
        with tab_carteira:
            ...
            st.divider()
            # Cabeçalho Ajustado com a Coluna de Dias (SLA)
            h = st.columns([1, 1.5, 1, 1, 1, 1, 1.2, 0.6, 0.4])
            col_names = ["**Data**", "**Comprador**", "**CPF**", "**Imóvel**", "**Valor**", "**Imobiliária**", "**Status (Alterar)**", "**Dias**", " "]
            for col, t in zip(h, col_names): col.write(t)
            ...
            with st.container(height=500):
                for i, r in df_f.iterrows():
                    ...
                    # 2. MUDAR STATUS (UPDATE EM TEMPO REAL)
                    ...
                    novo_status = c[6].selectbox(" ", lista_status, index=idx_atual, key=f"st_{i}", label_visibility="collapsed")
                    
                    if novo_status != r.iloc[7]:
                        df.at[i, df.columns[7]] = novo_status
                        conn.update(spreadsheet=URL_PLANILHA, data=df[df.columns[:8]])
                        st.cache_data.clear()
                        st.rerun()

                    # 3. CÁLCULO DE DIAS (SLA)
            ...
            except: st.warning("Erro ao gerar Excel. Verifique requirements.txt")
