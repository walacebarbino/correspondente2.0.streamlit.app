...
with st.sidebar.form("form_cadastro"):
    ...
    # 3º item: Atenção total às colunas de entrada conforme sua planilha
    ...
    # Inclusão do item: Enquadramento
    ...
    if st.form_submit_button("Cadastrar"):
        st.info("Para salvar permanentemente, adicione os dados no seu Excel do OneDrive.")
...
if not df.empty:
    ...
    # --- 4. GESTÃO DA CARTEIRA (CONFORME 1000155553) ---
    st.divider()
    ...
    for index, row in df.iterrows():
        ...
        if c[6].button("🗑️", key=f"del_{index}"):
            st.warning("Exclua a linha no Excel para remover do sistema.")
else:
    # Aviso de erro conforme sua imagem
