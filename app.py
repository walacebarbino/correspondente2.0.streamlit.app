    ...
    if not df.empty:
        ...
        with tab_bi:
            ...
            with c1:
                ...
                st.plotly_chart(px.line(df_mes, x='MÊS', y='Qtd', markers=True), use_container_width=True)
            with c2:
        ...
        with tab_carteira:
            ...
            with col_f3: busca = st.text_input("🔍 Buscar Comprador/Imobiliária")

            df_v = df.copy()
            if s_sel != "Todos": df_v = df_v[df_v['Status'] == s_sel]
            if e_sel != "Todos": df_v = df_v[df_v['Enquadramento'] == e_sel]
            if busca: df_v = df_v[df_v['Nome do Comprador'].str.contains(busca, case=False) | df_v['Imobiliária'].str.contains(busca, case=False)]
